...
- **Vendored into docx4j (Apache-safe, checked in):**
  - **W3C MathML Test Suite** (March 2010; W3C 3-clause BSD + W3C Document
    Licence — Apache-compatible). Presentation-MathML inputs for the import
    direction and MathML-parser robustness. MathML-only, so no OMML pairs; we
    author the expected OMML.
