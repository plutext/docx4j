...
public class GujaratiScriptProcessor extends IndicScriptProcessor {
    ...
    /** logging instance */
    private static final  Logger log = LoggerFactory.getLogger(GujaratiScriptProcessor.class);
    ...
    private static class GujaratiSyllabizer extends DefaultSyllabizer {
        ...
        // D := ( C N? H )?
        private static int isDeadConsonant(int[] ca, int s, int e) {
            if (s < 0) {
                return -1;
            } else {
                int c;
        ...
        private static int isLiveConsonant(int[] ca, int s, int e) {
            if (s < 0) {
                return -1;
            } else {
                int c;
