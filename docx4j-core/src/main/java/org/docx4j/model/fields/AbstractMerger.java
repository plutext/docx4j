...
public abstract class AbstractMerger {
	...
	private static Logger log = LoggerFactory.getLogger(AbstractMerger.class);		
	
	protected WordprocessingMLPackage input;
	
	protected AbstractMerger(WordprocessingMLPackage input) {
		this.input=input;
	}
	 ...
	 * @since 11.5.12
	 */
	public WordprocessingMLPackage getConsolidatedResultCrude( 
		...
		List<List<Object>> mdpResults = performOverList( input.getMainDocumentPart().getContent(), data, formTextFieldNames );
...
//				+ input.getMainDocumentPart().getXML() + "\n");
		List<Object> mdpResults = performOnInstance( input.getMainDocumentPart().getContent(), data, formTextFieldNames );
	 ...
	 * @throws Docx4JException 
	 */
	private List<List<Object>> performOverList( 
		...
		for (Map<DataFieldName, String> datamap : data) {
			...
			results.add(
					performOnInstance(contentList, datamap, formTextFieldNames));
		}
		
		return results;
	}
	
	protected abstract List<Object> performOnInstance( 
			List<Object> contentList, 
...
//    	}
     ...
     * @param fieldFate
     */
    public static void setOutputField(OutputField fieldFate) {
    	AbstractMerger.fieldFate = fieldFate;
    }
