...
docx4j's rules therefore split three ways today:

* four rules key on the **mode** (§1.4 of the rules doc lists them, and §5 below has the
  code sites);
* a handful of rules read a **named setting** (`w:defaultTabStop`, the four hyphenation
  settings, `w:evenAndOddHeaders`, `w:mirrorMargins`, `w:gutterAtTop`, `w:decimalSymbol`,
  `w:themeFontLang`, and exactly one `w:compat` flag,
  `w:doNotUseHTMLParagraphAutoSpacing`);
* everything else is unread.

**The policy this document is written to support** (to be implemented in a follow-up
batch): *a rule keys on the flag; the mode supplies the flag's default where the document
does not state it.* So `growAutofit` is read from `w:compat/w:growAutofit` when it is
there, and defaults to on below mode 14 and off at 14 and 15 when it is not - one lookup,
one place, and a mode-12 document that happens to have been resaved without its default
block still behaves like a mode-12 document.
...
and is the one re-keying candidate among them.
...
| `allowSpaceOfSameStyleInTable` | "Allow Contextual Spacing of Paragraphs in Tables" | [§3.5](word-layout-rules.md#s35) (the "only paragraph of a cell" rule) | IGNORED - docx4j cancels contextual space at cell edges unconditionally | 0 | yes: a cell of contextual paragraphs against docDefaults `w:after`, both ways |
...
| `autofitToFirstFixedWidthCell` | "Allow Table Columns To Exceed Preferred Widths of Constituent Cells" | [§6.3grid](word-layout-rules.md#s63grid), §6.4 | IGNORED - the grid-outranks-`w:tcW` rule is unconditional | 0 | yes: a table whose row 1 `w:tcW` disagree with `w:tblGrid`, both ways |
...
| `doNotAutofitConstrainedTables` | "Do Not AutoFit Tables To Fit Next To Wrapped Objects" | §6.3, [§6.8](word-layout-rules.md#s69) | IGNORED | 0 | low |
...
| `doNotBreakWrappedTables` | "Do Not Allow Floating Tables To Break Across Pages" | [§6.8](word-layout-rules.md#s69) | IGNORED - neither the float nor the positioned container carries a keep | 8 | yes: a tall `w:tblpPr` table straddling a page boundary |
| `doNotExpandShiftReturn` | "Don't Justify Lines Ending in Soft Line Break" | §4.2, [§2.5brline](word-layout-rules.md#s25brline), §4.4 | NO RULE - and docx4j behaves as if it were **always on**: `BrWriter` makes a `w:br` a nested `fo:block`, so the line before it is a last line and FOP does not justify it | 21 | **yes, high**: a justified paragraph ending in `w:br`, with and without the flag |
...
| `doNotUseIndentAsNumberingTabStop` | "Ignore Hanging Indent When Creating Tab Stop After Numbering" | [§2.8ind](word-layout-rules.md#s28ind), [§2.8wide](word-layout-rules.md#s28wide), §4.4 (the implicit stop a hanging indent makes) | IGNORED | 0 | **yes, high**: a numbered paragraph with a hanging indent and a later `w:tab`, both ways |
...
| `forgetLastTabAlignment` | "Ignore Width of Last Tab Stop When Aligning Paragraph If It Is Not Left Aligned" | [§4.4jc](word-layout-rules.md#s44jc), [§4.4jcboth](word-layout-rules.md#s44jcboth) | IGNORED - §4.4jc's whole-line alignment is exactly what this flag turns off | 5 | **yes, high**: the `tab-jc` probe with the flag |
| `growAutofit` | "Allow Tables to AutoFit Into Page Margins" | §6.3, [§6.5](word-layout-rules.md#s65pct), `GRID_OVERHANG_LIMIT` | IGNORED - and a candidate for §6.5's unresolved over-wide-grid disagreement | 15 | **yes, high**: the `table-grid-overwide` probe with the flag |
| `layoutRawTableWidth` | "Ignore Space Before Table When Deciding If Table Should Wrap Floating Object" | [§6.8](word-layout-rules.md#s69) | IGNORED | 5 | low |
...
| `noLeading` | "Do Not Add Leading Between Lines of Text" | §2.1, §2.3, [§2.7](word-layout-rules.md#s27alias) | IGNORED - `WordLineMetrics` always adds `tmExternalLeading` | 0 | **yes**: `line-auto`'s four fonts with the flag |
...
| `noTabHangInd` | "Do Not Create Custom Tab Stop for Hanging Indent" | §4.4 (the implicit stop at the left indent), [§2.8ind](word-layout-rules.md#s28ind) | IGNORED | 0 | yes, with `doNotUseIndentAsNumberingTabStop` |
...
| `suppressSpBfAfterPgBrk` | "Do Not Use Space Before On First Line After a Page Break" | [§3.3](word-layout-rules.md#s33) | **MODE-DEFAULTED** - the rule drops the space from mode 15 and never reads the flag (`WordLayoutFixups.mergePageBreakParagraphs`, `compatibilityMode < 15`) | 0 | yes: `spacing-page-top` with the flag, to settle the polarity |
...
| `useWord2002TableStyleRules` | "Emulate Word 2002 Table Style Rules" | [§7hftblstyle](word-layout-rules.md#s7hftblstyle), §6.3 (`ParagraphStylesInTableFix`) | IGNORED | 14 | medium: a table style with conditional formats, both ways |
...
| `overrideTableStyleFontSizeAndJustification` | Whether the default paragraph style's size and `w:jc` override a table style's | §6.3, [§7hftblstyle](word-layout-rules.md#s7hftblstyle) | **HONOURED** - `DocumentSettingsPart.overrideTableStyleFontSizeAndJustification`, applied in `convert/out/common/preprocess/ParagraphStylesInTableFix`. **The rules doc does not mention it** | 293 | tested (`PStyle*InTable*Test`); the rules doc should say so |
...
| `themeFontLang` | Which theme font (`minorHAnsi`/`minorEastAsia`/`minorBidi`) a run resolves to | §5.1, §5.2 | **HONOURED** - `org.docx4j.model.PropertyResolver`. **The rules doc does not mention it** | - | none needed; the rules doc should say so |
...
Ordered by corpus count against the importance of the rule the flag would switch.

1. **`growAutofit`** (15) - §6.3 / §6.5. Table column widths are the largest remaining
...
6. **`suppressSpBfAfterPgBrk`** (0 stated) - [§3.3](word-layout-rules.md#s33). The one
   mode-keyed rule that has a flag; re-keying it is a one-line change and settles the
   polarity question the rule leaves open.
7. **`doNotUseIndentAsNumberingTabStop`** + **`noTabHangInd`** (0 stated) - §2.8, §4.4.
   List-label geometry is measured to the glyph in §2.8; these two say whether the hanging
   indent makes a stop at all.
8. **`doNotBreakWrappedTables`** (8) - [§6.8](word-layout-rules.md#s69).
9. **`allowSpaceOfSameStyleInTable`** (0 stated) - [§3.5](word-layout-rules.md#s35). The
   cell contextual-spacing rule cost 37 Word pages against 43 on one document; the flag
   switches it.
10. **`noLeading`** (0 stated) - §2.1. It changes the pitch of every line in the document,
...
1. **`doNotExpandShiftReturn`** (21 stated). Word justifies a line ending in a soft line
   break unless this is set; docx4j never does, because `BrWriter` writes the break as a
   nested `fo:block` and the line before it is therefore a block's last line. So docx4j
   has the *legacy* behaviour for every document, including the 293 at mode 14/15.
   **Probe**: a justified paragraph of two full lines whose first ends in `<w:br/>`, in one
   document with `<w:doNotExpandShiftReturn/>` and one without; measure the first line's
   right edge.
...
## 5. Where the reading should live

A suggestion, not a decision.

Today the mode is read in four places, each with its own try/catch and its own default:
`TableWriter.compatibilityMode` (returns 12 on failure), `AbstractFOExporter` (passes it
into `WordLayoutFixups.apply`), `WordLayoutFixups.lineBoxAttributes` and
`.mergePageBreakParagraphs` (take it as an `int` parameter), and
`DocumentSettingsPart.getCompatibilityMode(pkg)` underneath them all. Adding fifty flags on
that pattern would mean fifty lookups scattered across `XsltFOFunctions`, `TableWriter`,
`WordLayoutFixups` and the layout managers.

Instead, one value object over `DocumentSettingsPart`, built once per conversion:

```java
package org.docx4j.model;   // beside HyphenationSettings, which is the same shape
...
public final class CompatibilityOptions {
    ...
    /** A w:compat flag: the document's value where it states one, else this mode's
     *  default (see word-layout-settings.md §2). */
    public boolean is(Flag flag);

    /** A w:compatSetting, by name. */
    public boolean setting(String name, boolean modeDefault);

    public enum Flag { GROW_AUTOFIT, DO_NOT_EXPAND_SHIFT_RETURN, NO_LEADING, ... }
}
```

Points that matter:

* **The mode default table lives in one place** - a per-`Flag` `int modeThreshold`, read as
  "on below this mode" - so §2 of this document has exactly one implementation.
* **`BooleanDefaultTrue` has three states** and the getter must not flatten them. This is
  the trap `org.docx4j.wml.AutospacingAccess` was written for
  ([§3.3](word-layout-rules.md#s33), the `w:beforeAutospacing="0"` override): XJC's
  `isX()` returns a primitive, so "absent" and "explicitly false" are indistinguishable
  through it. A flag stated as `w:val="0"` must beat the mode default; a flag not stated
  at all must not.
* **It is `HyphenationSettings`'s twin.** That class already does exactly this for the four
  hyphenation settings - a small immutable value read once from the package and passed into
  `WordLayoutFixups.apply` and on to `fo:root` as `docx4j:` attributes. `CompatibilityOptions`
  should ride the same route: built in `AbstractFOExporter` beside
  `HyphenationSettings.of(...)`, passed to `WordLayoutFixups.apply` in place of the bare
  `int compatibilityMode`, and put on `FOConversionContext` so `XsltFOFunctions` and
  `TableWriter` reach it without a second package lookup.
* **Flags the layout managers need travel as `docx4j:` attributes on `fo:root`**, as
  `docx4j:space-shrink`, `docx4j:hyphenation-zone`, `docx4j:hyphen-limit` and
  `docx4j:hyphenate-caps` already do (rules doc, Appendix). `forgetLastTabAlignment`,
  `doNotExpandShiftReturn`, `noLeading` and `wrapTrailSpaces` are all line-manager
  business and belong there; `growAutofit`, `useWord2002TableStyleRules` and the table
  flags are read while the FO is written and never need to reach FOP.
* **`DocumentSettingsPart` keeps only the raw accessors.** Its existing
  `getCompatibilityMode`, `isDoNotUseHTMLParagraphAutoSpacing`, `isAutoHyphenation`,
  `getHyphenationZone`, `getConsecutiveHyphenLimit`, `isDoNotHyphenateCaps`,
  `overrideTableStyleFontSizeAndJustification` and `getWordCompatSetting` are the right
  layer; the mode-defaulting logic does not belong in the OPC part.
* **Nothing changes behaviour on its own.** The first commit should be the class plus its
  unit tests, with every existing rule still reading the mode exactly as it does now; each
  re-keying in §4(a) is then its own measured change, against the corpus, one flag at a
  time.
