...
public class FORMTEXTMerger {
	...
	public static WordprocessingMLPackage getConsolidatedResultCrude(WordprocessingMLPackage input, 
		...
		List<List<Object>> mdpResults = performOverList(input, input.getMainDocumentPart().getContent(), data );
	...
	private static List<Object> performOnInstance(WordprocessingMLPackage input, 
		...
		for (FieldRef fr : fieldRefs) {
			...
			if ( fr.getFldName().equals("FORMTEXT") ) {
				...
				String datafieldName = getDatafieldNameFromFFData(fr.getFormFieldProperties());
...
//                }
	 ...
	 * @param ffData
	 */
	private static String getDatafieldNameFromFFData(CTFFData ffData) {
...
//    		log.debug("parent: " + ((Child)needToBeRemoved).getParent().getClass().getName());
    ...
    public static void setOutputField(OutputField fieldFate) {
    	FORMTEXTMerger.fieldFate = fieldFate;
    }
...
//						System.getProperty("user.dir") + "/template.docx"));
		...
		if (mergedOutput) {
			...
			FORMTEXTMerger.setOutputField(OutputField.REMOVED); // works
			FORMTEXTMerger.setOutputField(OutputField.AS_FORMTEXT_REGULAR); // works
			
//			System.out.println(XmlUtils.marshaltoString(wordMLPackage.getMainDocumentPart().getJaxbElement(), true, true));
			
			WordprocessingMLPackage output = FORMTEXTMerger.getConsolidatedResultCrude(wordMLPackage, data, true);
...
//			System.out.println(XmlUtils.marshaltoString(output.getMainDocumentPart().getJaxbElement(), true, true));
			...
			for (Map<DataFieldName, String> thismap : data) {
				FORMTEXTMerger.performMerge(wordMLPackage, thismap, true);
