...
/**
 * This class stores a list of the paragraph properties; each of these is
...
public final class ParagraphProperties {
	 ...
	 * 
	 	<w:pPr>
			<w:pStyle w:val="DivisionHeading"/>
		</w:pPr>
	...
	protected ParagraphProperties(Element e) {
				
	    Iterator elementIterator = e.elementIterator();
	    while(elementIterator.hasNext()){
	      Element element = (Element)elementIterator.next();
	      
		  if (element.getName()=="pStyle" ) {
			  pStyle = new PStyle(element);
		      properties.put( "pStyle", pStyle );			  
		  } else {
	...
	public Element marshall() {		
		...
		Element pPr = new DefaultElement(new QName(Constants.PARAGRAPH_PROPERTIES_TAG_NAME,
				Namespaces.namespaceWord));
		
		List children = pPr.content();

		Iterator it = properties.keySet().iterator();
		while(it.hasNext()) {
			Object o = properties.get( it.next() );
			if ( o instanceof PStyle ) {
				children.add( ((PStyle)o).marshall() );
			} else {
