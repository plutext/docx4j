...
public class BindingTraverserXSLT extends BindingTraverserCommonImpl {
	...
	static {
		try {
			Source xsltSource = new StreamSource(ResourceUtils.getResourceViaProperty(
	 ...
	 */
	public Object traverseToBind(JaxbXmlPart part, org.docx4j.openpackaging.packages.OpcPackage pkg,
			Map<String, org.opendope.xpaths.Xpaths.Xpath> xpathsMap) throws Docx4JException {
		...
		try {
			...
			// so we can use part.unmarshal, which should create a binder where
			// possible
			...
			transformParameters.put("bindingTraverserState", bindingTraverserState); // new
																						// for
																						// 3.3.0

			// Set up XPath "cache"; substantially quicker than Xalan XPath for
			// many lookups in large XML data files
			// Our strategy is to try the cache first (if enabled),
			// then if there is a cache miss, use
			// org.apache.xpath.CachedXPathAPI
			// (which is quicker than default javax.xml.xpath.XPath
			// implementations)
			if (ENABLE_XPATH_CACHE) {
				...
				if (domToXPathMap == null /* should be passed from ODH */) {
					...
					// CustomXmlPart cxp =
					// pkg.getCustomXmlDataStorageParts().get(xp.getDataBinding().getStoreItemID().toLowerCase());
					// System.out.println("mycxp: " + cxp.getClass().getName());
					// org.docx4j.openpackaging.parts.CustomXmlDataStoragePart
					// cdsp = (CustomXmlDataStoragePart)cxp;

					// We're only caching the first one we encounter
					// (even though, in principle, there could be multiple)
					CustomXmlPart cxp = CustomXmlDataStoragePartSelector
							.getCustomXmlDataStoragePart((WordprocessingMLPackage) pkg);

					if (cxp == null) {
						log.warn("No CustomXmlDataStoragePart found; can't cache.");
						/*
						 * TODO: would fail on StandardisedAnswersPart since
						 * that extends
						 * JaxbCustomXmlDataStoragePart<org.opendope.answers.
						 * Answers>
						 */
			...
			//
			// org.w3c.dom.Document docResult =
			...
			// } else
			{
				try {
					...
					return unmarshal(((org.w3c.dom.Document) result.getNode()), Docx4jProperties.getProperty(
	...
	/**
	 * Unmarshal a node using Context.jc, WITHOUT fallback to pre-processing in
	 * case of failure.
	 * 
	 * @param n
	 * @return
	...
	/**
	 * Workaround for the fact that Xalan doesn't let us pass an AtomicInteger
	 * into an extension function. Instead, it converts it into an int, which
	 * means the object in our bookmarkIdCounter parameter isn't updated.
	 * 
	...
	public static void log(ExpressionContext expressionContext, String message) {
		...
		// log.info(
		// com.sun.org.apache.xalan.internal.lib.NodeInfo.lineNumber(expressionContext
		// ) + " " + message);
		// com.sun.org.apache hell
		// but that only gives line number of input XML anyway, whereas more
		// useful is
		// currently executing line number of XSLT. ErrorListener seems to know
		// this? Explore some time...
	...
	public static void logXml(NodeIterator nodeIterator) {
		// Has different method, to prevent Xalan preferring the String log
		// method
		log(nodeIterator);
	}
	...
	private static final String		placeholderResource			= "OpenDoPE/placeholder.xml";						// default,
																													// can
																													// be
																													// overridden
																													// since
	...
	/**
	 * Calling code should set w:sdtPr/w:showingPlaceholder (ie bind.xslt), so
	 * RemovalHandler can do the right thing for Quantifier.ALL_BUT_PLACEHOLDERS
	 * case.
	 * 
	 * bind.xslt inserts the correct element structure for a simple bind, so all
	 * we do here is return the w:r element.
	 * 
	 * @param rPr
	 * @param sdtParent
	 * @return
	...
	protected static DocumentFragment createPlaceholder(RPr rPr) throws Exception {
		return createPlaceholder(rPr, "p"); // this returns the w:r, which
											// bind.xslt then wraps as
											// appropriate.
	}
	...
	protected static DocumentFragment createPlaceholder(RPr rPr, String sdtParent) throws Exception {
		...
		if (sdtParent.equals("p")) {
			...
			if (rPr == null) {
				...
				// Specific formatting
				// Note that changing the stylename will affect
				// Quantifier.ALL_BUT_PLACEHOLDERS processing
	...
	private static void createPlaceholderFragment() throws Exception {
		...
		try {
			...
			log.info(
					"No resource on classpath for property docx4j.model.datastorage.placeholder; falling back to using org/docx4j/model/datastorage/placeholder.xml");
	...
	private static void createPlaceholderBytes() throws Exception {
		...
		try {
			...
			log.info(
					"No resource on classpath at docx4j.model.datastorage.placeholder; falling back to using org/docx4j/model/datastorage/placeholder.xml");
			is = ResourceUtils.getResource(placeholderResourceFallback);
	 ...
	 *
	 * Note that the input XHTML must be suitable for the context ie you can't
	 * insert block level stuff (eg p) into a run level sdt.
	 * 
	 * This method requires docx4j-XHTMLImport.jar (LGPL) and its dependencies
	 * in order to function.
	 */
	public static DocumentFragment convertXHTML(BindingTraverserState bindingTraverserState,
			WordprocessingMLPackage pkg, JaxbXmlPart sourcePart, Map<String, CustomXmlPart> customXmlDataStorageParts,
			// String storeItemId, String xpath, String prefixMappings,
			Map<String, org.opendope.xpaths.Xpaths.Xpath> xpathsMap, NodeIterator sdtPrNodeIt, String sdtParent,
			String contentChild,
			// NodeIterator rPrNodeIt,
			// String tag,
			Map<String, Integer> sequenceCounters, BookmarkCounter bookmarkCounter) {
		...
		/*
		 * Find setBookmarkIdNext method. It's not part of the interface until
		 * 3.3.0, so use reflection
		 */
		...
		if (method == null) {
			log.info(
					"setBookmarkIdNext method not found.  If you are using docx4j-ImportXHTML v3.2.1 or later, it should be present.");
		} else {
			try {
		...
		if (bindingTraverserState.tcStack.peek() != null) {
			...
			BindingTraverserTableHelper.setupMaxWidthAndStyleForTc(bindingTraverserState.tblStack.peek(),
		...
		// Xpath xpath = xPathsPart.getXPathById(xPathsPart.getJaxbElement(),
		// xpathId);
		...
		} else if (log.isDebugEnabled() && r.trim().length() == 0) {
			// fallback removed for further speed improvement since we are
			// comfortable there are no "cache query"
			r = BindingHandler.xpathGetString(pkg, customXmlDataStorageParts, storeItemId, xpathExp, prefixMappings);
		...
		try {
			...
			if (r == null || r.trim().equals("")) {
				// sdtPr.setShowingPlcHdr(true); altering here doesn't work;
				// must do it in XSLT.
			...
			if (r.startsWith("<span")) {
				// Wrap the XHTML in a span element with @class, @style as
				// appropriate
				// so FS uses suitable CSS

				// Code copied from XsltHTMLFunctions.createBlockForRPr
				Style defaultRunStyle = (pkg.getMainDocumentPart().getStyleDefinitionsPart(false) != null
						? pkg.getMainDocumentPart().getStyleDefinitionsPart(false).getDefaultCharacterStyle()
						: null);

				String defaultCharacterStyleId;
				if (defaultRunStyle.getStyleId() == null) // possible, for non
															// MS source docx
			...
			} else if (Docx4jProperties
					.getProperty("docx4j.model.datastorage.BindingTraverser.XHTML.Block.rStyle.Adopt", false)) {

				log.debug("Block.rStyle.Adopt..");

				// its block level, and we're instructed to apply the paragraph
				// style
			...
			// Method setHyperlinkStyleMethod =
			// xhtmlImporterClass.getMethod("setHyperlinkStyle", String.class);
			...
			try {
				...
				// xhtmlImporterClass.getMethod("convert", String.class,
				// String.class, WordprocessingMLPackage.class );
				// results = (List<Object>)convertMethod.invoke(null, r,
				// baseUrl, pkg);
			...
			if (results.size() > 0 && results.get(0) instanceof P && sdtParent.equals("p")) {
				// Importer class always returns run-level content wrapped in a
				// w:p
				// so extract contents
				...
				for (Object o : ((P) results.get(0)).getContent()) {
					...
					// ((R)o).setRPr(rPrSDT);
					// } else {
					// log.debug("CSS rPR: " + XmlUtils.marshaltoString(cssRPR,
					// true, true));
				...
				for (Object o : results) {
					...
					if (sdtParent.equals("p") && o instanceof P) {
						if (log.isWarnEnabled()) {
							log.warn("DISCARDING conversion result (can't add in context p): "
	 ...
	 * @return
	 */
	private static DocumentFragment xhtmlError(String sdtParent, org.w3c.dom.Document docContainer,
			DocumentFragment docfrag, String errMsg) {
	 ...
	 */
	public static DocumentFragment xpathGenerateRuns(BindingTraverserState bindingTraverserState,
			WordprocessingMLPackage pkg, JaxbXmlPart sourcePart, Map<String, CustomXmlPart> customXmlDataStorageParts,
			Map<String, org.opendope.xpaths.Xpaths.Xpath> xpathsMap, NodeIterator sdtPrNodeIt, String sdtParent,
			String contentChild, boolean multiLine, BookmarkCounter bookmarkCounter) {
		...
		log.debug("Looking for xpath with id: " + xpathId + " referenced from part "
				+ sourcePart.getPartName().getName() + " at " + odTag);

		Xpath xpath = null;
		try {
			// xpath = xPathsPart.getXPathById(xPathsPart.getJaxbElement(),
			// xpathId);
			xpath = xpathsMap.get(xpathId);

		} catch (InputIntegrityException iie) {
			throw new InputIntegrityException("Couldn't find xpath with id: " + xpathId + " referenced from part "
					+ sourcePart.getPartName().getName() + " at " + odTag, iie);

			// Could fallback to trying to use the databinding sdtPr, but would
			// need to pass that in
		}
		if (xpath == null) {
			log.error("Couldn't find xpath with id: " + xpathId + " referenced from part "
					+ sourcePart.getPartName().getName() + " at " + odTag);
		...
		return xpathGenerateRuns(bindingTraverserState.getPathMap(), pkg, sourcePart, customXmlDataStorageParts,
				storeItemId, xpathExp, prefixMappings, sdtPr, sdtParent, contentChild, multiLine, bookmarkCounter);
	}
	...
	public static DocumentFragment xpathGenerateRuns(BindingTraverserState bindingTraverserState,
			WordprocessingMLPackage pkg, JaxbXmlPart sourcePart, Map<String, CustomXmlPart> customXmlDataStorageParts,
			String storeItemId, String xpath, String prefixMappings, NodeIterator sdtPrNodeIt, String sdtParent,
			String contentChild, boolean multiLine, BookmarkCounter bookmarkCounter) {
		...
		return xpathGenerateRuns(bindingTraverserState.getPathMap(), pkg, sourcePart, customXmlDataStorageParts,
				storeItemId, xpath, prefixMappings, sdtPr, sdtParent, contentChild, multiLine, bookmarkCounter);
	}

	/**
	 * Massage an XPath into the form it is in in our cache, so a hit is likely.
	 * For example, finding[6][1]/assets[1]/asset[32][1] to
	 * finding[6]/assets[1]/asset[32]
	 * 
	 * @param xpIn
	 * @return
	 */
	...
	public static DocumentFragment xpathGenerateRuns(Map<String, String> pathMap, WordprocessingMLPackage pkg,
			JaxbXmlPart sourcePart, Map<String, CustomXmlPart> customXmlDataStorageParts, String storeItemId,
			String xpath, String prefixMappings, SdtPr sdtPr, String sdtParent, String contentChild, boolean multiLine,
		 ...
		 * multiline data with w:multiLine absent or set to 0 ie false - cases
		 * with and without rPr - inline and block level sdt
		 */
		...
		try {
			...
			for (Object o : sdtPr.getRPrOrAliasOrLock()) {
				o = XmlUtils.unwrap(o); // Sun/Oracle JAXB (recent versions?)
										// wraps RPR in JAXBElement
				if (o instanceof RPr) {
					rPr = (RPr) o;
					break;
				}
			...
			return BindingHandler.getValueInserterPlainText().toOpenXml(dataBinding, rPr, multiLine, bookmarkCounter, r,
					sourcePart);
	...
	public static DocumentFragment xpathInjectImage(WordprocessingMLPackage wmlPackage, JaxbXmlPart sourcePart,
			Map<String, CustomXmlDataStoragePart> customXmlDataStorageParts, String storeItemId, String xpath,
			String prefixMappings, String sdtParent, String contentChild, String cx, String cy) {
		...
		// for all parts for which it is to be called (eg mdp, header parts
		// etc).
	 ...
	 * @since 3.0.1
	 */
	public static String xpathInjectImageRelId(WordprocessingMLPackage wmlPackage, JaxbXmlPart sourcePart,
			Map<String, CustomXmlDataStoragePart> customXmlDataStorageParts,
			Map<String, org.opendope.xpaths.Xpaths.Xpath> xpathsMap, String tag) {
		...
		return xpathInjectImageRelId(wmlPackage, sourcePart, customXmlDataStorageParts, storeItemId, xpathExp,
				prefixMappings);
	}
	...
	public static String getRepeatPositionCondition(Map<String, org.opendope.xpaths.Xpaths.Xpath> xpathsMap,
			String odTag) {
	...
	public static DocumentFragment xpathDate(WordprocessingMLPackage wmlPackage, JaxbXmlPart sourcePart,
			Map<String, CustomXmlPart> customXmlDataStorageParts, NodeIterator sdtPrNodeIt, String sdtParent,
			String contentChild, NodeIterator dateNodeIt) {
		...
		try {
			...
			/*
			 * <w:date w:fullDate="2012-08-19T00:00:00Z"> <w:dateFormat
			 * w:val="d/MM/yyyy"/> <w:lid w:val="en-AU"/> <w:storeMappedDataAs
			 * w:val="dateTime"/> <w:calendar w:val="gregorian"/> </w:date>
			 * 
			 * Assume our String r contains something like
	...
	/**
	 * Convert the FlatOPC into an AltChunk, which Xalan can insert into XSLT
	 * output.
	 * 
	 * @since 3.0.1
	 */
	public static DocumentFragment convertFlatOPC(WordprocessingMLPackage pkg, JaxbXmlPart sourcePart,
			Map<String, CustomXmlPart> customXmlDataStorageParts,
			// String storeItemId, String xpath, String prefixMappings,
			Map<String, org.opendope.xpaths.Xpaths.Xpath> xpathsMap, String sdtParent, String contentChild,
			NodeIterator rPrNodeIt, String tag) {
		...
		try {
			...
			// Xpath xpath =
			// xPathsPart.getXPathById(xPathsPart.getJaxbElement(), xpathId);
			Xpath xpath = xpathsMap.get(xpathId);
			...
			String r = BindingHandler.xpathGetString(pkg, customXmlDataStorageParts, storeItemId, xpathExp,
					prefixMappings);
			if (r == null)
				return nullResultParagraph(sdtParent, "[missing!]");
			if (!r.startsWith("<?xml")) {
				 ...
				 * Word can't open it without this! <?mso-application
				 * progid="Word.Document"?> // optional
				 */
	 ...
	 * @since 3.2.2
	 */
	public static DocumentFragment w14Checkbox(WordprocessingMLPackage wmlPackage, JaxbXmlPart sourcePart,
			Map<String, CustomXmlPart> customXmlDataStorageParts, NodeIterator sdtPrNodeIt, String sdtParent,
			String contentChild) {
		...
		/*
		 * <w14:checkbox> <w14:checked w14:val="0"/> <w14:checkedState
		 * w14:val="2612" w14:font="MS Gothic"/> <w14:uncheckedState
		 * w14:val="2610" w14:font="MS Gothic"/> </w14:checkbox>
		 */
		...
		try {
			...
			/*
			 * <w:p> <w:r> <w:rPr> <w:rFonts w:ascii="MS Gothic"
			 * w:eastAsia="MS Gothic" w:hAnsi="MS Gothic" w:hint="eastAsia"/>
	...
	private static PartName getNewPartName(String prefix, String suffix, RelationshipsPart rp)
