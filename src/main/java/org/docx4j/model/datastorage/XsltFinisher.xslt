...
<!--  In this XSLT, you can apply arbitrary transformations to your OpenXML.
		...
		w:sdtPr/w:tag/@w:val='od:call=xyz'
		...
		<xsl:when
			...
			<xsl:choose>
				...
				<xsl:when test="$templateName = 't_r'">
					<xsl:call-template name="t_r" />
				</xsl:when>
<!-- 				<xsl:when test="$templateName = 't_c'">
					<xsl:call-template name="t_c" />
				</xsl:when>
				<xsl:when test="$templateName = 't_db'">
					<xsl:call-template name="t_db" />
				</xsl:when> --> 
				<xsl:otherwise>
  ...
  <xsl:template name="t_c">  

			<xsl:comment>It worked!</xsl:comment>

  </xsl:template>
  
  <xsl:template name="t_r">  
	...
	<xsl:choose>
		...
		<xsl:when test="$res = '$30'">
		    <xsl:copy>
		      <xsl:apply-templates select="@*|node()" mode="t_r_Action"/> <!--  use a different mode to do what we want -->
		    </xsl:copy>			
		</xsl:when>
		<xsl:otherwise>
		    <xsl:copy>
  ...
  <xsl:template name="t_db">  

			<xsl:comment>It worked!</xsl:comment>

  </xsl:template>
  
    <xsl:template match="/ | @*|node()" mode="t_r_Action">
    <xsl:copy>
      <xsl:apply-templates select="@*|node()" mode="t_r_Action"/>
    </xsl:copy>
  </xsl:template>
  
  <!--  in this example, color the row  
   -->
    <xsl:template match="w:tcPr" mode="t_r_Action">
