...
/**
 * Formats the string value of the field according to the three possible
 * formatting switches:
 * 
 * + date-and-time-formatting-switch: \@ + numeric-formatting-switch: \# +
 * general-formatting-switch: \*
 * 
 * Note that the general-formatting-switch arguments CHARFORMAT and MERGEFORMAT
 * are not handled here. It is the responsibility of the calling code to handle
 * these.
 * 
...
public class FormattingSwitchHelper {
	...
	private static Logger												log							= LoggerFactory
			.getLogger(FormattingSwitchHelper.class);

	/*
	 * http://office.microsoft.com/en-us/word-help/insert-and-format-field-codes
	 * -in-word-2010-HA101830917.aspx Switches:
	 * 
	 * Format switches (\*): Capitalization formats \* Caps Capitalizes the
	 * first letter of each word. \* FirstCap Capitalizes the first letter of
	 * the first word. \* Upper Capitalizes all letters. \* Lower Capitalizes
	 * none of the result; all letters are lowercase.
	 *
	 * Number formats \* alphabetic Lowercase alphabetic characters (a, b, c).
	 * \* ALPHABETIC Uppercase alphabetic characters (A, B, C). \* Arabic Arabic
	 * cardinal numerals (1, 2, 3). \* ArabicDash Only PAGE! Arabic cardinal
	 * numerals surrounded by hyphen characters (- 1 -, - 2 -, - 3 -). \*
	 * CardText Lowercase Cardinal text (can be combined with capitalization
	 * formats). (one, two, three). \* DollarText ........ For example, { = 9.20
	 * + 5.35 \* DollarText \* Upper } displays FOURTEEN AND 55/100. \* Hex
	 * Hexadecimal numbers. \* OrdText Lowercase Ordinal text (can be combined
	 * with capitalization formats). (first, second, third) \* Ordinal Ordinal
	 * Arabic numerals (1st, 2nd, 3rd). \* roman Lowercase Roman numerals (i,
	 * ii, iii). \* ROMAN Uppercase Roman numerals (I, II, III).
	 * 
	 * Character formats and protecting previously applied formats \* Charformat
	 * This switch applies the formatting of the first letter of the field name
	 * to the entire result. \* MERGEFORMAT This switch applies the formatting
	 * of the previous result to the new result.
	 * 
	 * Numeric format switch (\#):
	 * 
	 * 0 (zero) This format item specifies the requisite numeric places to
	 * display in the result. If the result does not include a digit in that
	 * place, Word displays a 0 (zero). # This format item specifies the
	 * requisite numeric places to display in the result. If the result does not
	 * include a digit in that place, Word displays a space. x This format item
	 * drops digits to the left of the "x" placeholder. If the placeholder is to
	 * the right of the decimal point, Word rounds the result to that place. .
	 * (decimal point) This format item determines the decimal point position.
	 * (Use the decimal symbol that is specified as part of the regional
	 * settings in Control Panel.) , (digit grouping symbol) This format item
	 * separates a series of three digits. (Use the digit grouping symbol that
	 * is specified as part of the regional settings in Control Panel.) - (minus
	 * sign) This format item adds a minus sign to a negative result or adds a
	 * space if the result is positive or 0 (zero). + (plus sign) This format
	 * item adds a plus sign to a positive result, a minus sign to a negative
	 * result, or a space if the result is 0 (zero). %, $, *, and so on This
	 * format item includes the specified character in the result. positive;
	 * negative[; zero] This format item specifies different number formats for
	 * a positive result, a negative result, and a 0 (zero) result, separated by
	 * semicolons. `numbereditem` This format item displays the number of the
	 * preceding item that you numbered by using the Caption command (References
	 * tab, Captions group) or by inserting a SEQ field.
	 *
	 * 
	 * 
	 * Date Time format switch (\@): Quotation marks are not required around
	 * simple date-time formats that do not include spaces or text
	 * 
	 * Month M Number without a leading 0 (zero) for single-digit months. MM
	 * Number with a leading 0 (zero) for single-digit months. MMM Three-letter
	 * abbreviation. MMMM Full name.
	 *
	 * Day d Day of the month as a number without a leading 0 (zero) for
	 * single-digit days. dd Day of the month as a number with a leading 0
	 * (zero) for single-digit days. ddd Day of the week as three-letter
	 * abbreviation. dddd Day of the week as its full name.
	 * 
	 * Year yy Year as two digits with a leading 0 (zero) for years 01 through
	 * 09. yyyy Year as four digits.
	 * 
	 * Hours h (12h) Hour without a leading 0 (zero) for single-digit hours. H
	 * (24h) Hour without a leading 0 (zero) for single-digit hours. hh (12h)
	 * Hour with a leading 0 (zero) for single-digit hours. HH (24h) Hour with a
	 * leading 0 (zero) for single-digit hours.
	 * 
	 * Minutes m Minutes without a leading 0 (zero) for single-digit minutes. mm
	 * Minutes with a leading 0 (zero) for single-digit minutes.
	 * 
	 * Seconds s Seconds without a leading 0 (zero) for single-digit seconds. ss
	 * Seconds with a leading 0 (zero) for single-digit seconds.
	 * 
	 * AM/PM am/pm or AM/PM Displays AM or PM as uppercase.
	 * 
	 * Other Date Time items 'text' Display any specified text in a date or
	 * time. Enclose the text in single quotation marks. (How do you escape a
	 * single quote? - The example of the page above causes an error in word...)
	 * character This format item includes the specified character in a date or
	 * time, such as a : (colon), - (hyphen), * (asterisk), or space.
	...
	static {
		...
		// Month
		DATE_FORMAT_ITEMS_TO_JAVA.put("M", "M"); // Number without a leading 0
													// (zero) for single-digit
													// months.
		DATE_FORMAT_ITEMS_TO_JAVA.put("MM", "MM"); // Number with a leading 0
													// (zero) for single-digit
													// months.
		DATE_FORMAT_ITEMS_TO_JAVA.put("MMM", "MMM"); // Three-letter
														// abbreviation.
		DATE_FORMAT_ITEMS_TO_JAVA.put("MMMM", "MMMM"); // Full name.
		// Day
		DATE_FORMAT_ITEMS_TO_JAVA.put("d", "d"); // Day of the month as a number
													// without a leading 0
													// (zero) for single-digit
													// days.
		DATE_FORMAT_ITEMS_TO_JAVA.put("dd", "dd"); // Day of the month as a
													// number with a leading 0
													// (zero) for single-digit
													// days.
		DATE_FORMAT_ITEMS_TO_JAVA.put("ddd", "EEE"); // Day of the week as
														// three-letter
														// abbreviation.
		DATE_FORMAT_ITEMS_TO_JAVA.put("dddd", "EEEE"); // Day of the week as its
														// full name.
		// Year
		DATE_FORMAT_ITEMS_TO_JAVA.put("yy", "yy"); // Year as two digits with a
													// leading 0 (zero) for
													// years 01 through 09.
		DATE_FORMAT_ITEMS_TO_JAVA.put("yyyy", "yyyy"); // Year as four digits.
		// Hour
		DATE_FORMAT_ITEMS_TO_JAVA.put("h", "K"); // (12h) Hour without a leading
													// 0 (zero) for single-digit
													// hours.
		DATE_FORMAT_ITEMS_TO_JAVA.put("H", "H"); // (24h) Hour without a leading
													// 0 (zero) for single-digit
													// hours.
		DATE_FORMAT_ITEMS_TO_JAVA.put("hh", "KK"); // (12h) Hour with a leading
													// 0 (zero) for single-digit
													// hours.
		DATE_FORMAT_ITEMS_TO_JAVA.put("HH", "HH"); // (24h) Hour with a leading
													// 0 (zero) for single-digit
													// hours.
		// Minute
		DATE_FORMAT_ITEMS_TO_JAVA.put("m", "m"); // Minutes without a leading 0
													// (zero) for single-digit
													// minutes.
		DATE_FORMAT_ITEMS_TO_JAVA.put("mm", "mm"); // Minutes with a leading 0
													// (zero) for single-digit
													// minutes.
		// Second
		DATE_FORMAT_ITEMS_TO_JAVA.put("s", "s"); // Seconds without a leading 0
													// (zero) for single-digit
													// seconds.
		DATE_FORMAT_ITEMS_TO_JAVA.put("ss", "ss"); // Seconds with a leading 0
													// (zero) for single-digit
													// seconds.
		// AM/PM
		// DATE_FORMAT_ITEMS_TO_JAVA.put("am/pm","a"); //Displays AM or PM as
		// uppercase.
		// DATE_FORMAT_ITEMS_TO_JAVA.put("AM/PM","a"); //Displays AM or PM as
		// uppercase.
	}
	...
	public static String applyFormattingSwitch(WordprocessingMLPackage wmlPackage, FldSimpleModel model, String value)
			throws Docx4JException {
		return applyFormattingSwitch(wmlPackage, model, value, null);
	}

	public static String applyFormattingSwitch(WordprocessingMLPackage wmlPackage, FldSimpleModel model, String value,
			String lang) throws Docx4JException {
		// date-and-time-formatting-switch: \@
		Date date = null;
		try {
			String dtFormat = findFirstSwitchValue("\\@", model.getFldParameters(), true);
			if (dtFormat == null) {
				// SPECIFICATION: If no date-and-time-formatting-switch is
				// present,
				// a date or time result is formatted in an
				// implementation-defined manner.

				// TODO .. analyse what Word does
				// for now, just leave as is
			} else if (dtFormat.equals("")) {
				// Verified with Word 2010 sp1 for DOCPROPERTY (very likely
				// others are the same)
		...
		if (date == null) {
			...
			if (nFormat != null) {
				...
				if (nFormat.equals("")) {
					return "Error! Switch argument not specified.";
					// Verified with Word 2010 sp1, for =, DOCPROPERTY,
					// MERGEFIELD
					...
					// SPECIFICATION: If no numeric-formatting-switch is
					// present,
					// a numeric result is formatted without leading spaces or
					// trailing fractional zeros.
					// If the result is negative, a leading minus sign is
					// present.
					// If the result is a whole number, no radix point is
					// present.
		...
		// does not correspond to the format specified, this switch has no
		// effect.
	...
	private static Date getDate(FldSimpleModel model, String dateStr) throws FieldResultIsNotADateOrTimeException {
		 ...
		 * 
		 * <vt:lpwstr>13/4/2013</vt:lpwstr> <vt:lpwstr>13/04/2013</vt:lpwstr>
		 * <vt:lpwstr>13/04/13</vt:lpwstr> <vt:lpwstr>13-04-2013</vt:lpwstr>
		 * <vt:lpwstr>13/04/2013 11:05 AM</vt:lpwstr>
		 * <vt:lpwstr>2013-08-19T00:00:00Z</vt:lpwstr>
		 * 
		 * Control Panel > Region & Language > Formats determines whether Word
		 * 2010 interprets 11/12 as 11 Dec, or 12 Nov.
		 * 
		...
		if (inputFormat == null) {
			...
			try {
				...
				throw new FieldResultIsNotADateOrTimeException(
						"Can't parse " + dateStr + " using format " + inputFormat);
			}
		}
	}

	private static double getNumber(WordprocessingMLPackage wmlPackage, FldSimpleModel model, String value)
	...
	private static String formatNumber(FldSimpleModel model, String wordNumberPattern, double dub, String lang)
		...
		/*
		 * Per the spec, if no numeric-formatting-switch is present, a numeric
		 * result is formatted without leading spaces or trailing fractional
		 * zeros. If the result is negative, a leading minus sign is present. If
		 * the result is a whole number, no radix point is present.
		 */
		...
		// in Java's NumberFormatter, you have # or 0, then a literal or %,$ etc
		// then # or 0 again
		boolean encounteredNonFiller = false;

		int round = 0;
		int fillerBeforeDecimalPointCount = 0; // to check for an anomolous
												// result
		...
		if ((wordNumberPattern != null) && (wordNumberPattern.length() > 0)) {
			...
			while (idx < wordNumberPattern.length()) {
				...
				if (ch == '\'') {
					if (inLiteral) {
						// End literal
						buffer.append(wordNumberPattern.substring(valueStart, idx)); // ignore
																						// closing
																						// '
						idx2 = idx + 1; // skip '
						/*
						 * Word treats the whitespace outside the right single
						 * quote as significant, and inserts zero, one or many
						 * spaces as if the whitespace was inside the literal.
						 * 
					...
					// if (lastCh != ch) {
					// if (valueStart > -1) {
					// appendNumberItem(buffer,
					// wordNumberPattern.substring(valueStart, idx));
					// }
					// valueStart = idx;
					// lastCh = ch;
					// }
					...
					if (ch == 'x') {
						...
						if (encounteredDecimalPoint) {
							...
							if (fillerAfterDecimalPoint == '\0') {
								...
								fillerAfterDecimalPoint = '#'; // and now we're
																// committed to
																// that
							} else {
		...
		if (fillerBeforeDecimalPointCount < 1 && (wordNumberPattern != null) && (wordNumberPattern.length() > 0)) {
			/*
		...
		try {
			if (lang != null) {
				...
				formatter = new DecimalFormat(javaFormatter, new DecimalFormatSymbols(localeforLanguageTag(lang)));
	...
	private static String formatGeneral(FldSimpleModel model, String format, String value) {
		// Note that the general-formatting-switch arguments CHARFORMAT and
		// MERGEFORMAT
		...
		// alphabetic, arabic, cardtext, dollartext, hex, ordtext, ordinal, or
		// roman
	 ...
	 * 
	 * @param wordName
	 *            word page number format
	 * @return null if the wordName is null, the correspondig fo value if
	 *         present or a default.
	 */
	...
	public static int getFoPageNumberDecoration(String wordName) {
		...
		if ((wordName != null) && (wordName.length() > 0)) {
			if ("ArabicDash".equals(wordName) || NumberFormat.NUMBER_IN_DASH.value().equals(wordName)) {
				ret = DECORATION_DASH;
			}
		}
		return ret;
	...
	public static String getFldSimpleName(String instr) {
		...
		if ((instr != null) && (instr.length() > 0)) {
			while ((startValue < instr.length()) && (instr.charAt(startValue) == ' '))
				startValue++;
			endValue = startValue;
			while ((endValue < instr.length()) && (instr.charAt(endValue) != ' '))
				endValue++;
	...
	public static String convertDatePattern(String wordDatePattern) {
		...
		if ((wordDatePattern != null) && (wordDatePattern.length() > 0)) {
			while (idx < wordDatePattern.length()) {
				...
				if (ch == '\'') {
					if (inLiteral) {
						buffer.append(wordDatePattern.substring(valueStart, idx)); // ignore
																					// closing
																					// '
						idx2 = idx + 1; // skip '
						while ((idx2 < wordDatePattern.length()) && (wordDatePattern.charAt(idx2) == ' ')) {
							buffer.append(' ');
							idx2++;
						}
						buffer.append('\'');
					...
					if (((ch == 'a') || (ch == 'A')) && (isAMPM(wordDatePattern, idx))) {
						buffer.append('a');
						idx += 4;
						lastCh = '\0';
						valueStart = -1;
	...
	private static boolean isAMPM(String wordDatePattern, int idx) {
		...
		return ((idx + 4 < wordDatePattern.length())
	...
	/**
	 * @param language
	 *            abbreviation like "fr-CA",
	 * @return SimpleDateFormat instance for specified language if null
	 *         will @return SimpleDateFormat.getDateTimeInstance()
	 */
	private static SimpleDateFormat getSimpleDateFormat(String lang) {
		...
		if (dateFormat == null) {
			// dateFormat =
			// (SimpleDateFormat)SimpleDateFormat.getDateTimeInstance(DateFormat.DEFAULT,
			// 0, Locale.forLanguageTag(lang));
