...
public class MailMerger {
     ...
     */
    public static WordprocessingMLPackage getConsolidatedResultCrude(WordprocessingMLPackage input, List<Map<DataFieldName, String>> data)
            throws Docx4JException {
        ...
        for (List<Object> content : mdpResults) {
            ...
            if (processHeadersAndFooters) {
                for (CTRel ctRel : hfTemplates.keySet()) {
                    ...
                    // Populate it
                    List<Object> newContent = performOnInstance(input, ((ContentAccessor) part).getContent(), data.get(i), formTextFieldNames);
        ...
        if (processHeadersAndFooters) {
            ...
            for (Relationship r : rp.getJaxbElement().getRelationship()) {
                ...
                if (r.getType().equals(Namespaces.HEADER) || r.getType().equals(Namespaces.FOOTER)) {
                    ...
                    List<Object> results = performOnInstance(input, ((ContentAccessor) part).getContent(), data, formTextFieldNames);
     ...
     * @throws Docx4JException
     */
    private static List<List<Object>> performOverList(WordprocessingMLPackage input, List<Object> contentList, List<Map<DataFieldName, String>> data,
            FormTextFieldNames formTextFieldNames) throws Docx4JException {

        List<List<Object>> results = new ArrayList<List<Object>>();
        for (Map<DataFieldName, String> datamap : data) {

            results.add(performOnInstance(input, contentList, datamap, formTextFieldNames));
        }

        return results;
    }

    private static List<Object> performOnInstance(WordprocessingMLPackage input, List<Object> contentList, Map<DataFieldName, String> datamap,
            FormTextFieldNames formTextFieldNames) throws Docx4JException {
