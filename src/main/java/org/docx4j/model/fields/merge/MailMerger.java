 ...
 * 
 * Instance values are merged into a docx containing MERGEFIELD to produce
 * output docx made up of a copy of the input docx for each collection of input
 * values.
 * 
 * The output can be a single docx, or multiple docx.
 * 
 * If you choose single docx, there are two ways to do this:
 * 
 * One is using MergeDocx, which will ensure each constituent "document" doesn't
 * affect the neighbouring ones (eg numbering will restart).
 * 
 * The other is the "poor man's" approach, which puts them together, and just
 * hopes for the best. Images and hyperlinks should be ok. But numbering will
 * continue, as will footnotes/endnotes.
 * 
 * From 3.0, there is some support for formatting switches (date/time, numeric,
 * and general), and basic support for MERGEFORMAT.
 * 
 * LIMITATIONS: - no support for text before (\b) and text after (\f) switches -
 * no support for \m and \v switches - no support for multiple MERGEFIELD in a
 * single instruction (eg MERGEFIELD CoutesyTitle \f " " MERGEFIELD FirstName \f
 * " " MERGEFIELD LastName )
 * 
 * @author jharrop
 *
...
public class MailMerger {
	...
	/**
	 * A "poor man's" approach, which generates the mail merge results as a
	 * single docx, and just hopes for the best. Images and hyperlinks should be
	 * ok. But numbering will continue, as will footnotes/endnotes.
	 * 
	 * @param input
	 * @param data
	 * @return
	 ...
	 */
	public static WordprocessingMLPackage getConsolidatedResultCrude(
			WordprocessingMLPackage input, List<Map<DataFieldName, String>> data)
			throws Docx4JException {
	 ...
	 * A "poor man's" approach, which generates the mail merge results as a
	 * single docx, and just hopes for the best. Images and hyperlinks should be
	 * ok. But numbering will continue, as will footnotes/endnotes. [Advert:] If
	 * this isn't working for you, the commercial Enterprise Edition of docx4j
	 * (MergeDocx component) will solve your problems.
	 * 
	 * @param input
	 * @param data
	 * @param processHeadersAndFooters
	 *            process headers and footers in FIRST section only. If you have
	 *            multiple sections in your input docx, performMerge is a better
	 *            approach
	 * @return
	 * @throws Docx4JException
	 *             @ since 2.8.1
	 */
	public static WordprocessingMLPackage getConsolidatedResultCrude(
			WordprocessingMLPackage input,
			List<Map<DataFieldName, String>> data,
			boolean processHeadersAndFooters) throws Docx4JException {
		...
		log.debug("complexified: "
				+ XmlUtils.marshaltoString(input.getMainDocumentPart()
						.getJaxbElement(), true));
		List<List<Object>> mdpResults = performOverList(input, input
				.getMainDocumentPart().getContent(), data, formTextFieldNames);
		...
		if (processHeadersAndFooters) {
			...
			for (CTRel rel : hdrFtrRefs) {
				...
				JaxbXmlPart part = (JaxbXmlPart) input.getMainDocumentPart()
						.getRelationshipsPart().getPart(relId);
				FieldsPreprocessor.complexifyFields(part);

				log.debug("complexified: "
						+ XmlUtils.marshaltoString(part.getJaxbElement(), true));
		...
		WordprocessingMLPackage target = WordprocessingMLPackage
				.load(new ByteArrayInputStream(template));
		...
		/*
		 * What we're doing, effectively, is doing the main content in a single
		 * hit (ie for all instances), and then, for each instance, doing the
		 * headers/footers.
		 * 
		 * It is this way because that is how the code has evolved.
		 * 
		 * Since we have to do the headers/footers instance by instance, it
		 * would probably be neater to do the main content at the same time (ie
		 * instead of using performOverList at the start of this method).
		 */
			...
			if (processHeadersAndFooters) {
				for (CTRel ctRel : hfTemplates.keySet()) {
					...
					if (part instanceof HeaderPart) {
						...
						clonedPart.setJaxbElement(Context.getWmlObjectFactory()
								.createHdr());
					...
					List<Object> newContent = performOnInstance(input,
							((ContentAccessor) part).getContent(), data.get(i),
							formTextFieldNames);
					((ContentAccessor) clonedPart).getContent().addAll(
							newContent);

					// Add it
					Relationship rel = target.getMainDocumentPart()
							.addTargetPart(clonedPart,
	...
	/**
	 * Word uses the existing sectPr element, but adds a page numbering restart
	 * to it. TODO: investigate what it does with headers/footers.
	 * 
	 * @param template
	 * @return
	 */
	private static SectPr getDocumentSeparator(WordprocessingMLPackage template) {
		...
		SectPr sectPr = template.getMainDocumentPart().getJaxbElement()
				.getBody().getSectPr();
		...
		if (sectPr == null) {
			...
			String landscapeString = Docx4jProperties.getProperties()
					.getProperty("docx4j.PageOrientationLandscape", "false");
	...
	// public static List<WordprocessingMLPackage>
	// getResults(WordprocessingMLPackage input,
	// List<Map<DataFieldName, String>> data) throws Docx4JException {
	//
	// List<WordprocessingMLPackage> pkgs = new
	// ArrayList<WordprocessingMLPackage>();
	//
	// // Just MDP for now
	// FieldsPreprocessor.complexifyFields(input.getMainDocumentPart() );
	// List<List<Object>> results =
	// performOverList(input.getMainDocumentPart().getContent(), data );
	//
	// // Prepare for cloning
	// OpcPackage result = null;
	//
	 ...
	 * 
	 * This is the best approach, if your input has headers/footers in multiple
	 * sections.
	 * 
	 * If you are using MergeDocx, you can use that to join the instances into a
	 * single docx.
	 * 
	 * WARNING: The input docx will be modified, so input a copy if that is a
	 * problem. This is left to the user, since that can potentially be more
	 * efficient, than doing it here.
	 * 
	 * @param input
	 * @param data
	 * @param processHeadersAndFooters
	...
	public static void performMerge(WordprocessingMLPackage input,
			Map<DataFieldName, String> data, boolean processHeadersAndFooters)
			throws Docx4JException {
		...
		FieldsPreprocessor.complexifyFields(input.getMainDocumentPart());
		// log.debug("\n\n COMPLEXIFIED " +
		// input.getMainDocumentPart().getPartName().getName() + "\n\n"
		// + input.getMainDocumentPart().getXML() + "\n");
		List<Object> mdpResults = performOnInstance(input, input
				.getMainDocumentPart().getContent(), data, formTextFieldNames);
		...
		if (processHeadersAndFooters) {
			...
			RelationshipsPart rp = input.getMainDocumentPart()
			...
			for (Relationship r : rp.getJaxbElement().getRelationship()) {
				...
				if (r.getType().equals(Namespaces.HEADER)
					...
					log.debug("\n\n BEFORE "
							...
							+ "\n\n"
							+ XmlUtils.marshaltoString(part.getJaxbElement(),
									true, true) + "\n");
	...
	/**
	 * The idea is to be able to perform a mail merge on content from main
	 * document part, or a header/footer etc.
	 * 
	 * We return a list of content lists, so that the consumer can choose
	 * whether to produce a single docx (via MergeDocx or otherwise), or a docx
	...
	private static List<List<Object>> performOverList(
			WordprocessingMLPackage input, List<Object> contentList,
			List<Map<DataFieldName, String>> data,
		...
		for (Map<DataFieldName, String> datamap : data) {
			...
			results.add(performOnInstance(input, contentList, datamap,
					formTextFieldNames));
		}
	...
	private static List<Object> performOnInstance(
			WordprocessingMLPackage input, List<Object> contentList,
		...
		for (FieldRef fr : fieldRefs) {
			...
			if (fr.getFldName().equals("MERGEFIELD")) {
				...
				if (StringUtils.isBlank(val)) {
					...
					try {
						fsm.build(instr);
						val = FormattingSwitchHelper.applyFormattingSwitch(
								input, fsm, val);

						gFormat = FormattingSwitchHelper.findFirstSwitchValue(
								"\\*", fsm.getFldParameters(), true);
						// Solely for potential use in
						// OutputField.AS_FORMTEXT_REGULAR
						// We are in fact applying all formatting switches
						// above.
				...
				if (fieldFate.equals(OutputField.AS_FORMTEXT_REGULAR)) {
					...
					log.debug(gFormat);
					// TODO if we're going to use gFormat, setup FSM
					// irrespective of whether we can find key
					...
					// replace instrText
					// eg MERGEFIELD CLIENT.ORGANIZATIONSTATE \* Upper \*
					// MERGEFORMAT
					// to FORMTEXT
					...
					if (o instanceof Text) {
						...
						log.error("TODO: set FORMTEXT in"
								+ o.getClass().getName());
						log.error(XmlUtils.marshaltoString(instructions.get(0),
								true, true));
						continue;
					}

					String fieldName = formTextFieldNames
							.generateName(datafieldName);
					log.debug("Field name normalisation: " + datafieldName
							+ " -> " + fieldName);
					setFormFieldProperties(fr, fieldName, null);

					// remove <w:highlight w:val="lightGray"/>, if present
					// (corresponds in Word to clicking Legacy Forms > Form
					// Field Shading)
					...
					// If doing an actual mail merge, the begin-separate run is
					// removed, as is the end run
	...
	protected static void canonicaliseStarts(ComplexFieldLocator fl,
		...
		for (P p : fl.getStarts()) {
			...
			if (p.getParent() instanceof ContentAccessor) {
				// 2.8.1
				index = ((ContentAccessor) p.getParent()).getContent().indexOf(
						p);
				P newP = FieldsPreprocessor.canonicalise(p, fieldRefs);
				log.debug("Canonicalised: "
						+ XmlUtils.marshaltoString(newP, true, true));
				...
				// 3.0.1
				index = ((CTTextbox) p.getParent()).getTxbxContent()
						.getContent().indexOf(p);
				P newP = FieldsPreprocessor.canonicalise(p, fieldRefs);
				log.debug("Canonicalised: "
						+ XmlUtils.marshaltoString(newP, true, true));
				((CTTextbox) p.getParent()).getTxbxContent().getContent()
						.set(index, newP);
			} else {
				throw new Docx4JException("Unexpected parent: "
						+ p.getParent().getClass().getName());
			}
		}
	}

	/**
	 * Get the datafield name from, for example <w:instrText
	 * xml:space="preserve"> MERGEFIELD Kundenstrasse \* MERGEFORMAT
	 * </w:instrText> or <w:instrText xml:space="preserve"> MERGEFIELD
	 * Kundenstrasse</w:instrText>
	 */
	protected static String getDatafieldNameFromInstr(String instr) {
		...
		if (tmp.startsWith("\"")) {
			if (tmp.indexOf("\"", 1) > -1) {
				...
				// hope for the best
				datafieldName = tmp.indexOf(" ") > -1 ? tmp.substring(1,
						tmp.indexOf(" ")) : tmp.substring(1);
			}
		} else {
			datafieldName = tmp.indexOf(" ") > -1 ? tmp.substring(0,
					tmp.indexOf(" ")) : tmp;
		}
	...
	protected static String extractInstr(List<Object> instructions) {
		...
		if (instructions.size() != 1) {
			 ...
			 * lasauv</w:instrText> </w:r> <w:r> <w:instrText
			 * xml:space="preserve">egarde \* MERGEFORMAT </w:instrText> </w:r>
			 * 
			 * for (Object i : instructions) { i = XmlUtils.unwrap(i); if (i
			 * instanceof Text) { log.error( ((Text)i).getValue()); } else {
	...
	/**
	 * Parse through all content inside the paragraph to concatenate all values
	 * inside a text
	 * 
	 * @param paragraph
	 *            The paragraph which contains (or not) data
			...
			if (content instanceof org.docx4j.wml.R) {
				...
				for (Object o2 : runContent) {
					if (o2 instanceof javax.xml.bind.JAXBElement) {
						if (((JAXBElement<?>) o2).getDeclaredType().getName()
							...
							result.append(((org.docx4j.wml.Text) o2).getValue());
						}
					}
				}
			}
	 ...
	 * 
	 * @param content
	 *            Body (or other part) of the template
	 * @param needToBeRemoved
	 *            The object that will be removed from the content
	 */
	protected static void recursiveRemove(ContentAccessor content,
		...
		for (Object object : content.getContent()) {
			if (object instanceof ContentAccessor) {
				...
				if (element.getValue() instanceof ContentAccessor) {
					recursiveRemove((ContentAccessor) element.getValue(),
							needToBeRemoved);
				}
			}
		}
	 ...
	 * Default is REMOVED.
	 * 
	 * KEEP_MERGEFIELD will allow you to perform another merge on the output
	 * document.
	 * 
	 * The AS_FORMTEXT options convert the MERGEFIELD to a FORMTEXT field. This
	 * is convenient if you want users to be able to edit the field, where
	 * editing is restricted to forms.
	 * 
	 * @param fieldFate
	 */
	...
	public enum OutputField {
		...
		DEFAULT, REMOVED, KEEP_MERGEFIELD, AS_FORMTEXT_REGULAR;
		// AS_FORMTEXT_TYPED,
		// AS_FORMTEXT_TYPED_FORMATTED;
	}

	protected static void setFormFieldProperties(FieldRef fr, String ffName,
		...
		JAXBElement<org.docx4j.wml.CTFFName> ffnameWrapped = wmlObjectFactory
		...
		JAXBElement<org.docx4j.wml.BooleanDefaultTrue> booleandefaulttrueWrapped = wmlObjectFactory
		...
		BooleanDefaultTrue booleandefaulttrue2 = wmlObjectFactory
				.createBooleanDefaultTrue();
		JAXBElement<org.docx4j.wml.BooleanDefaultTrue> booleandefaulttrueWrapped2 = wmlObjectFactory
		...
		JAXBElement<org.docx4j.wml.CTFFTextInput> fftextinputWrapped = wmlObjectFactory
				.createCTFFDataTextInput(fftextinput);
		...
		if (ffTextInputFormat != null) {
			...
			CTFFTextInput.Format fftextinputformat = wmlObjectFactory
					.createCTFFTextInputFormat();
	...
	/**
	 * If we're converting MERGEFIELD to FORMTEXT, it is desirable to make the
	 * w:fldChar/w:ffData/w:name unique within the docx (though Word 2010 can
	 * still open the docx if they aren't), and to remove spaces
	 * 
	 * @author jharrop
	 * @since 3.0.0
	 */
	protected static class FormTextFieldNames {
		...
		// MS-OE376 says Word only allows strings of length at most 40 for the
		// name attribute.

		// Also, by experiment (in Word 2010), only alphanumberic characters and
		// "_" are allowed
		...
		// says:
		// If multiple bookmarks in a document share the same name, then the
		// first bookmark
		// (defined by the location of the bookmarkStart element in document
		// order) shall
		// be maintained, and all subsequent bookmarks should be ignored.

		// By experiment (inserting a cross ref) it looks like Word 2010
		// actually ignores the first!
		 ...
		 * http://www.w3.org/TR/css3-fonts/#font-family-casing
		 */
		private static class FormTextFieldNameSet extends HashSet<String> {
			...
			// not @Override because that would require the key parameter to be
			// of type Object
