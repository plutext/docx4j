 ...
 * 
 * This class is a refactoring of LoadFromZipFile, which couldn't read from an
 * input stream
 * 
 * @author jharrop
 * 
...
public class LoadFromZipNG extends Load {
	...
	// public HashMap<String, ByteArray> partByteArrays = new HashMap<String,
	// ByteArray>();
	...
	// String partName)
	private RelationshipsPart getRelationshipsPartFromZip(Base p, HashMap<String, ByteArray> partByteArrays,
		...
		try {
			...
			// thePart = new RelationshipsPart( p, new PartName("/" + partName),
			// is );
	...
	private static InputStream getInputStreamFromZippedPart(HashMap<String, ByteArray> partByteArrays, String partName)
			throws IOException {
	...
	/*
	 * recursively (i) create new Parts for each thing listed in the
	 * relationships (ii) add the new Part to the package (iii) cross the
	 * PartName off unusedZipEntries
	 */
	// private void addPartsFromRelationships(ZipFile zf, Base source,
	// RelationshipsPart rp)
	private void addPartsFromRelationships(HashMap<String, ByteArray> partByteArrays, Base source, RelationshipsPart rp,
			ContentTypeManager ctm) throws Docx4JException {
		...
		//
		// } catch (Exception e) {
		// throw new Docx4JException("Failed to add parts from relationships",
		// e);
		// }
		// }
		...
		for (Relationship r : rp.getRelationships().getRelationship()) {
			...
			log.debug("\n For Relationship Id=" + r.getId() + " Source is " + rp.getSourceP().getPartName()
					+ ", Target is " + r.getTarget() + ", type: " + r.getType());
	...
	/**
	 * Get a Part (except a relationships part), and all its related parts. This
	 ...
	 */
	// private void getPart(ZipFile zf, Package pkg, RelationshipsPart rp,
	// Relationship r)
	private void getPart(HashMap<String, ByteArray> partByteArrays, OpcPackage pkg, RelationshipsPart rp,
			Relationship r, ContentTypeManager ctm) throws Docx4JException, InvalidFormatException, URISyntaxException {
		...
		if (r.getType().equals(Namespaces.HYPERLINK)) {
			...
			// resolvedPartUri = (new
			// java.net.URI(resolvedPartUri)).normalize().toString();
			...
			if (loadExternalTargets && r.getType().equals(Namespaces.IMAGE)) {
				...
				log.info(
						"Encountered (but not loading) external resource " + r.getTarget() + " of type " + r.getType());
			}
			return;
		}
		...
		part = getRawPart(partByteArrays, ctm, resolvedPartUri, r); // will
																	// throw
																	// exception
																	// if null
	...
	/**
	 * Get the Relationships Part (if there is one) for a given Part. Otherwise
	 * return null.
	 * 
	 * @param zf
	 * @param part
	...
	public RelationshipsPart getRelationshipsPart(HashMap<String, ByteArray> partByteArrays, Part part)
		...
		try {
			try {
				...
				if (part instanceof org.docx4j.openpackaging.parts.ThemePart) {
					...
					// ((org.docx4j.openpackaging.parts.JaxbXmlPart)part).unmarshal(
					// is );
					...
					try {
						...
						if (o instanceof CoverPageProperties) {
							...
							is.reset();
							data.setDocument(is); // Not necessarily JAXB,
													// that's just our method
													// name
						...
						CustomXmlDataStorage data = getCustomXmlDataStorageClass().factory();
						is.reset();
						data.setDocument(is); // Not necessarily JAXB, that's
												// just our method name
						((org.docx4j.openpackaging.parts.CustomXmlDataStoragePart) part).setData(data);
					}
					...
					// Experimental 22/6/2011; don't fall back to binary (which
					// we used to)
					...
					// part = getBinaryPart(partByteArrays, ctm,
					// resolvedPartUri);
	...
	// public static Part getBinaryPart(ZipFile zf, ContentTypeManager ctm,
	// String resolvedPartUri)
	public static Part getBinaryPart(HashMap<String, ByteArray> partByteArrays, ContentTypeManager ctm,
			String resolvedPartUri) throws Docx4JException {
