 ...
 * 
 * This class doesn't care how the parts are physically stored (that is
 * PartLoader's problem).
 * 
 * What this class knows how to do is to traverse the opc, via its
 * relationships. It would be possible/interesting to have a design which lazily
 * followed the rels, but we don't do that. All rel parts are unmarshalled and
 * traversed when invoked.
 * 
 * @author jharrop
 * 
 */
public class Load3 extends Load {
	...
	/**
	 * By setting this, you can load the contents into this existing package (ie
	...
	public OpcPackage get() throws Docx4JException {
		...
		if (existingPkg == null) {
			...
			if (existingPkg.getClass() != p.getClass()) {
				throw new Docx4JException("Can't fill " + existingPkg.getClass().getName() + " with "
						+ p.getClass().getName() + " contents.");
			}
	...
	private void getPart(OpcPackage pkg, RelationshipsPart rp, Relationship r, ContentTypeManager ctm)
		...
		part = getRawPart(ctm, resolvedPartUri, r); // will throw exception if
													// null
		...
		try {
			try {
				...
				if (part instanceof org.docx4j.openpackaging.parts.ThemePart
					...
					// Note that this is done lazily, since the below lines are
					// commented out
					...
					// ContentTypeManager initially detects them as
	...
	// "/sample-docs/word/FontEmbedded.docx";
