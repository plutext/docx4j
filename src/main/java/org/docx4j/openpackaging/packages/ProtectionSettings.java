...
/**
 * The Protection Settings which are common across docx, pptx, xlsx, namely mark
 * as final, encrypt with password, and digital signature. Subclasses implement
 * the docx and xlsx format specific features.
 * 
 * @author jharrop
 * @since 3.3.0
 */
public abstract class ProtectionSettings {
	...
	public void setMarkAsFinal(boolean val) {
		...
		// First
		if (pkg.getDocPropsCustomPart() == null && val) // only create it if
														// we're setting the
														// value
		{
		...
		// Second
		if (pkg.getDocPropsExtendedPart() == null && val) // only create it if
															// we're setting the
															// value
		{
			pkg.addDocPropsExtendedPart();
		}
		setDocSecurity(0); // surprising, but this is what Word 2013 does (you'd
							// expect 2)

		// Third: <cp:contentStatus>Final</cp:contentStatus>
		if (pkg.getDocPropsCorePart() == null && val) // only create it if we're
														// setting the value
		{
	...
	/**
	 * Note, this won't create the DocPropsExtendedPart (app.xml) if it doesn't
	 * exist
	 * 
	 * @param val
	 */
	...
	// Encrypt with Password - this is implemented in OpcPackage's load & save
	// methods
	...
	// *
	// * Encrypt with Password functionality is implemented in OpcPackage's load
	// and save methods.
	// *
	// * @return
	// */
	...
	// */
	// public void sign(InputStream PKCS12stream, String password) throws
	// Docx4JException {
	//
	// Signing signing = null;
	// try {
	// Class<?> signingClass =
	// Class.forName("com.plutext.dsig.SignatureHelper");
	// Constructor<?> ctor = signingClass.getConstructor(OpcPackage.class);
	// signing = (Signing) ctor.newInstance(this.pkg);
	// } catch (Exception e) {
	// log.warn("Docx4j Enterprise jar v3.3 or greater not found. Required for
	// Digital Signatures.");
	// throw new Docx4JException("missing Enterprise version required for
	...
	/**
	 * get the SignatureHelper object, so you can sign the package using custom
	 * settings.
	 * 
	 * @param certificateIS
	 * @param password
	...
	public Object getSignatureHelper() throws Docx4JException {
		...
		try {
			...
			throw new Docx4JException(
					"Docx4j Enterprise jar v3.3 or greater not found. Required for Digital Signatures.");
		}
	}
