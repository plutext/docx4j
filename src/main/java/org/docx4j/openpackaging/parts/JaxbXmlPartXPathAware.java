...
public abstract class JaxbXmlPartXPathAware<E> extends JaxbXmlPart<E> implements XPathEnabled<E> {
	 ...
	 * 
	 * In theory, modifications can be made to either the infoset preserving
	 * view or the JAXB representation of the document while the other view
	 * remains unmodified. The binder ought to be able to synchronize the
	 * changes made in the modified view back into the other view using the
	 * appropriate Binder update methods, #updateXML(Object, Object) or
	 * #updateJAXB(Object).
	 * 
	 * But JAXB doesn't currently work as advertised .. access to this object is
	 * offered for advanced users on an experimental basis only.
	 */
	...
	/**
	 * You can't use this override to create/update a binder, since this would
	 * set the jaxbElement field to something different to the object being
	 * passed in (as a consequence of the process to create a binder).
	 * 
	...
	/**
	 * Set the JAXBElement for this part, and a corresponding binder, based on
	 * the object provided. Returns the new JAXBElement, so calling code can
	 * manipulate it. Beware that this object is different to the one passed in!
	 * 
	 * @param source
	 * @return
	 ...
	 * @since 3.0.0
	 */
	public List<JAXBAssociation> getJAXBAssociationsForXPath(String xpathExpr, boolean refreshXmlFirst)
			throws JAXBException, XPathBinderAssociationIsPartialException {
	...
	/**
	 * Fetch DOM node / JAXB object pairs matching an XPath (for example
	 * ".//w:p" - note the dot, which is necessary for this sort of relative
	 * path).
	 * 
	 * In JAXB, this association is partial; not all XML elements have
	 * associated JAXB objects, and not all JAXB objects have associated XML
	 ...
	 * @since 3.0.0
	 */
	public List<JAXBAssociation> getJAXBAssociationsForXPath(Object someJaxbElement, String xpathExpr,
	...
	/**
	 * Unmarshalling via DOM document can be 4x slower than unmarshalling the
	 * inputstream using XMLStreamReader, so we avoid doing that where possible.
	 * 
	 * @param is
	 * @param forceBinder
	 * @return
	...
	private E unmarshal(java.io.InputStream is, boolean forceBinder) throws JAXBException {
		...
		try {
			...
			try {
				...
				if (!forceBinder) {
					if (this instanceof MainDocumentPart) {
						wantBinder = Docx4jProperties.getProperty(
								"docx4j.openpackaging.parts.JaxbXmlPartXPathAware.binder.eager.MainDocumentPart",
								false);
					} else {
						wantBinder = Docx4jProperties.getProperty(
				...
				if (wantBinder) {
					...
					if (Context.jaxbImplementation == JAXBImplementation.ECLIPSELINK_MOXy) {
						 ...
						 * 
						 * I can set a ValidationEventHandler on an
						 * Unmarshaller, which triggers on an unexpected element
						 * ValidationEvent, then catch an UnmarshalException to
						 * handle that. That all works as expected :-)
						 * 
						 * If instead I set a ValidationEventHandler on a Binder
						 * then call unmarshal, an unexpected element
						 * ValidationEvent never triggers.
						 * 
						 * I tested MOXy 2.5.2 and 2.6.3.
						 * 
						 * Looking at the MOXy source code on GitHub, at a high
						 * level it looks like it ought to work.
						 * 
						 * JAXBBinder lets you setEventHandler on XMLBinder, and
						 * that in turn invokes setErrorHandler on the
						 * unmarshallers.
						 */
						...
						if (contents.contains("AlternateContent")) {
							...
							// 3.4.0: this needs to be refined, since we can now
							// handle
							// alternate content in w:r (so than in itself is
							// ok)
					...
					{
						doc = XmlUtils.getNewDocumentBuilder().parse(is); // this
																			// also
																			// guards
																			// against
																			// XXE
					...
					unwrapUsually(binder, doc); // unlikely to need this in the
												// code below
					...
					xif.setProperty(XMLInputFactory.SUPPORT_DTD, false); // a
																			// DTD
																			// is
																			// merely
																			// ignored,
					...
					// XMLStreamReaderWrapper xsrw = new
					// XMLStreamReaderWrapper(this, xsr)
				 ...
				 * 10; DOCTYPE is disallowed when the feature
				...
				if (ue instanceof UnmarshalException) {
					...
					if (((UnmarshalException) ue).getLinkedException() != null) {
						...
						if (((UnmarshalException) ue).getLinkedException().getMessage().contains("entity")) {
							 ...
							 * ParseError at [row,col]:[10,19] Message: The
							 * entity "xxe" was referenced, but not declared. at
						...
						if (((UnmarshalException) ue).getLinkedException().getMessage()
								.contains("EmptyPrefixedAttName?prefix=\"xmlns")) {
							// eg xmlns:ns0=""

							/*
							 * Caused by: org.xml.sax.SAXParseException;
							 * lineNumber: 2; columnNumber: 600; The value of
							 * the attribute
							 * "prefix="xmlns",localpart="ns0",rawname="xmlns:
							 * ns0"" is invalid. Prefixed namespace bindings may
							 * not be empty. at
							 * com.sun.org.apache.xerces.internal.parsers.
							 * DOMParser.parse(DOMParser.java:257) at
							 * com.sun.org.apache.xerces.internal.jaxp.
							 * DocumentBuilderImpl.parse(DocumentBuilderImpl.
							 * java:339) at
				...
				if (is.markSupported()) {
					...
					/*
					 * Always try our preprocessor, since if what is first
					 * encountered is w14:whatever the error would be:
					 * 
					...
					try {
						...
						// drop things we don't have a content model for; the
						// preprocessor doesn't need to
						// do that
						 ...
						 * encountered with
						 * /src/test/resources/jaxb-binder-issue.docx See
						 * http://old.nabble.com/BinderImpl.associativeUnmarshal
						 * -ClassCastException-casting-to-JAXBElement-td32456585
						 * .html and http://java.net/jira/browse/JAXB-874
						 * 
						 * java.lang.ClassCastException: org.docx4j.wml.PPr
						 * cannot be cast to javax.xml.bind.JAXBElement at
						 * com.sun.xml.internal.bind.v2.runtime.
						 * ElementBeanInfoImpl$IntercepterLoader.intercept(
						 * Unknown Source) at
						 * com.sun.xml.internal.bind.v2.runtime.unmarshaller.
						 * UnmarshallingContext.endElement(Unknown Source) at
						 * com.sun.xml.internal.bind.v2.runtime.unmarshaller.
						 * InterningXmlVisitor.endElement(Unknown Source) at
						 * com.sun.xml.internal.bind.v2.runtime.unmarshaller.
						 * SAXConnector.endElement(Unknown Source) at
						 * com.sun.xml.internal.bind.unmarshaller.DOMScanner.
						 ...
						 * com.sun.xml.internal.bind.unmarshaller.DOMScanner.
						 * scan(Unknown Source) at
						 * com.sun.xml.internal.bind.v2.runtime.BinderImpl.
						 * associativeUnmarshal(Unknown Source) at
						 * com.sun.xml.internal.bind.v2.runtime.BinderImpl.
						 * unmarshal(Unknown Source) at
						 * org.docx4j.openpackaging.parts.WordprocessingML.
						 * MainDocumentPart.unmarshal(MainDocumentPart.java:321)
						 */
			...
			// System.out.println("Time taken " + diff + " for " +
			// this.getPartName().getName() );
			 ...
			 * startDTD(Unknown Source) at
			 * com.sun.org.apache.xerces.internal.impl.XMLDTDScannerImpl.
			 * scanDTDInternalSubset(Unknown Source) at
			 * com.sun.org.apache.xerces.internal.impl.
			 * XMLDocumentScannerImpl$DTDDriver.dispatch(Unknown Source) at
			 * com.sun.org.apache.xerces.internal.impl.
			 * XMLDocumentScannerImpl$DTDDriver.next(Unknown Source) at
			 * com.sun.org.apache.xerces.internal.impl.
			 * XMLDocumentScannerImpl$PrologDriver.next(Unknown Source)
			 */
	...
	public E unmarshal(org.w3c.dom.Element el) throws JAXBException {
		...
		try {
			...
			if (Context.jaxbImplementation == JAXBImplementation.ECLIPSELINK_MOXy) {
				...
				// Always transform; rather than converting to String, testing
				// that etc
				try {
					Templates mcPreprocessorXslt = JaxbValidationEventHandler.getMcPreprocessor();
					DOMResult result = new DOMResult();
			...
			try {
				...
				unwrapUsually(binder, el); // unlikely to need this in the code
											// below
	...
	// private String toString(ByteBuffer bb) throws
